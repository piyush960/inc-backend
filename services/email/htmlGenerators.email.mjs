...
const judgeAllocationTemplate = readFileSync(__dirname + '/views/emails/judgeAllocation.email.ejs', 'utf8')
...
async function sendAllocationEmail(event_name, projects, judge) {
    try {
        return ejs.render(judgeAllocationTemplate, { data: { event_name, projects, judge }, filename: __dirname + '/views/emails/judgeAllocation.email.ejs', cache: true, async: true })
    } catch (err) {
        throw err
    }
}
