...
function ticketValidation() {
    return [
        cookie('ticket', 'Error while handling ticket').if(param('event_name').equals(eventsName[2])).optional().customSanitizer((_, { req }) => req.signedCookies.ticket).trim().isLength({ min: 17, max: 18 }).escape().isAlphanumeric('en-US', { ignore: '-' }).withMessage('Invalid ticket in cookies'),
    ]
}
